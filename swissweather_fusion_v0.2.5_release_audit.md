...
# 15. v0.2.5 — convective physics, provider metadata, deprecations

**Test suite 604 → 634. pyflakes clean.**

A release with a thesis rather than a changelog: Model B gains an
instability input, and several fields the providers were already sending
stop being discarded.

## 15.1 SWF-025-001 — CAPE was available all along

`const.py` and `models/model_b.py` both recorded that CAPE *"was hoped
for but turned out to require a paid tier"*. That conclusion was drawn
from Meteonomiqs' `/forecast2` endpoint and then **carried for several
releases without being re-checked against the other providers**.

Open-Meteo's MeteoSwiss documentation lists **CAPE and convective
inhibition as free hourly variables** for all three ICON models, on the
same request already being made. Also free and previously unrequested:
`freezing_level_height`, `snowfall_height`, `cloud_base`.

**What this changes.** Model B was a rain-APPROACH detector — station
pressure/humidity tendency plus upwind radar sampling — with nothing
describing whether the atmosphere could actually convect. It could not
distinguish steady frontal rain arriving from the southwest from a
thunderstorm developing overhead. That limitation was stated plainly when
the model's honesty was questioned; it is now addressed rather than
documented.

**Design decisions worth stating:**

- **CIN gates CAPE rather than scaling it.** High CAPE under a strong cap
  is the classic false-alarm case — the energy is present and nothing can
  reach it. Scaling would still leave a signal; suppression is correct,
  because a convective warning that ignores the lid cries wolf on every
  warm afternoon, which is how a warning becomes ignored.
- **Unknown CIN does not suppress.** The same asymmetry as radar quality
  (P1-16): treating unknown as capped would silently discard the entire
  contribution whenever a provider omits one field.
- **Instability alone cannot fire the trigger.** All three CAPE band
  probabilities sit below `V0_TRIGGER_PROBABILITY` by design. CAPE
  describes *potential*, not an event in progress; it should support a
  radar or tendency signal, never substitute for one.
- **max(), not a sum.** Three independent lines of evidence for one
  event. Summing would let three weak signals manufacture a strong one.
- **CAPE fuses with max, not mean** — like a wind gust it is a hazard
  indicator, and averaging away one model's warning is the wrong
  direction to be wrong in. CIN fuses toward the *least* capped model,
  consistently pessimistic.

**Honest caveat.** The band thresholds (300 / 1000 / 2500 J/kg) are
conventional operational figures, not tuned for this location — a v0
heuristic exactly like the radar thresholds. `storm_events` now
accumulates, so they are measurable rather than permanently assumed.

## 15.2 SWF-025-002 — provider confidence (Class D)

meteoblue reports an hourly `predictability` score. It has been parsed
into `ParsedMeteoblueForecast` since v0.1.x and **thrown away ever
since** — the architecture document's Class D ("provider metadata") gap,
never built. A source telling us it is unsure about a particular hour is
information, and it is now stored and surfaced.

## 15.3 SWF-025-004 — daily cloud coverage

The daily aggregation computed cloud cover to feed the condition resolver
and then discarded it, so the returned dict carried only temperature,
precipitation and condition. An automation reading
`daily.cloud_coverage` silently received a template default rather than
an error — the worst way for a field to be missing, since it looks like
it works.

Found while porting a real watering automation, which could not be ported
because of it.

## 15.4 SWF-025-005 — deprecated Home Assistant APIs

Baseline for this project is HA 2026.9 with no backward-compatibility
requirement, so the deprecated aliases are simply removed:

| Was | Now |
| --- | --- |
| `helpers.entity.EntityCategory` | `homeassistant.const.EntityCategory` |
| `helpers.entity.DeviceInfo` | `helpers.device_registry.DeviceInfo` |
| `AddEntitiesCallback` | `AddConfigEntryEntitiesCallback` |

A test asserts the deprecated import paths do not reappear.

Checked and found clean: `hass.helpers`, `async_get_registry`,
`DEVICE_CLASS_*` constants, `SUPPORT_*` magic numbers,
`async_setup_platforms`, `device_state_attributes`, and
`OptionsFlow.config_entry` assignment. The project's `utcnow()` is its
own helper returning an aware datetime, not the deprecated
`datetime.utcnow()`.

## 15.5 UI exposure

Several new parameters are **not members of Home Assistant's Forecast
contract**, so no weather card can render them regardless of
configuration. Each therefore gets its own entity — the mechanism the
architecture review identified for exactly this case (AR-01):

Convective energy (CAPE), Convective inhibition, Freezing level,
Snowfall level, Cloud base, Forecast confidence (diagnostic), and
**Convective risk**.

Convective risk is deliberately **separate from the headline storm
score**. The storm score is the maximum of tendency, radar and
instability, so a high value could mean rain already arriving *or* an
unstable atmosphere with nothing happening yet. Those call for different
responses and one number cannot express both. The sensor reports 0 when
capped, with a `capped` attribute explaining why.

## 15.6 Verification

Four changes were reverted and the suite re-run: **four tests fail**,
covering CIN suppression, the three-way max, daily cloud coverage and the
capped-state sensor. Restored and re-verified at 634.

The reachability guard added in v0.2.2 earned itself again: it failed
immediately when the new parameters were registered but not yet added to
the blend's measurement set — the exact "implemented but not wired"
class that cost four previous releases.

## 15.7 Still outstanding

- **UV and ozone** need Open-Meteo's Air Quality API; `uv_index` is not
  offered for the MeteoSwiss models at all, which is why it stayed blank.
- **SRF symbol codes** remain namespaced. Mapping them to Home Assistant
  conditions needs SRF's own code table, which this project has not
  verified — and inventing a mapping from a guessed table is precisely
  the CombiPrecip filename mistake.
- Season-boundary cold start, outlier rejection in reconciliation, the
  metadata API for true model run times, and the unwired lapse-rate
  pre-correction.
